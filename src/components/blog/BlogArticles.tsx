...
const BlogArticles: React.FC<BlogArticleProps> = ({ navigation }) => {
  ...
  const calculateVisibleIndex = (event: {
    ...
    const viewableIndex = Math.floor(
      event.nativeEvent.contentOffset.x / (screenWidth - 26)
    );
    ...
    <View style={styles.blogArticlesContainer}>
      ...
      <FlatList
        ...
        snapToOffsets={[
          ...
          2 * (screenWidth - 16),
          3 * (screenWidth - 16),
        ]} // Offset positions for each card
        ...
        renderItem={({ item, index }) => (
          <BlogCarouselCard data={carouselCardData} navigation={navigation} />
        )}
      ...
      <View style={styles.carouselIcons}>
        <Circle
          color={activeCardIndex === 0 ? COLORS.brandBlue : COLORS.placeHolder}
          size={5}
          fill={activeCardIndex === 0 ? COLORS.brandBlue : COLORS.placeHolder}
        />
        <Circle
          color={activeCardIndex === 1 ? COLORS.brandBlue : COLORS.placeHolder}
          size={5}
          fill={activeCardIndex === 1 ? COLORS.brandBlue : COLORS.placeHolder}
        />
        <Circle
          color={activeCardIndex === 2 ? COLORS.brandBlue : COLORS.placeHolder}
          size={5}
          fill={activeCardIndex === 2 ? COLORS.brandBlue : COLORS.placeHolder}
        />
        <Circle
          color={activeCardIndex === 3 ? COLORS.brandBlue : COLORS.placeHolder}
          size={5}
          fill={activeCardIndex === 3 ? COLORS.brandBlue : COLORS.placeHolder}
        />
      </View>
